...
- Each order shows:
  ...
  - Status (e.g. OPEN, CLOSED, REFUNDED).
  - Date and time.
- For OPEN orders, staff can jump to the payment screen to charge the customer.
- For completed orders that are not yet refunded, staff can trigger a refund where needed.
- If there are no orders, the page clearly informs the user that no orders are found.
